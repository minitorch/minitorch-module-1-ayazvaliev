...
def test_sigmoid(a: float, b: float) -> None:
    ...
    * It is  strictly increasing.
    """
    a_sig = sigmoid(a)
    assert 0 <= a_sig and a_sig <= 1
    assert_close(1 - a_sig, sigmoid(-a))
    assert_close(sigmoid(0), 0.5)
    assert (not (a > b)) or (a_sig >= sigmoid(b))
...
def test_transitive(a: float, b: float, c: float) -> None:
    ...
    first_stat = lt(a, b) and lt(b, c)
    second_stat = lt(a, c)

    assert (not first_stat) or second_stat
...
def test_other(a: float) -> None:
    ...
    # TODO: Implement for Task 0.2.
    a_relu = relu(a)
    assert a_relu == a if a > 0 else a_relu == 0
